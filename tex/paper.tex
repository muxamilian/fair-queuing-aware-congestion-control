...
If there is fair queuing with fq\_codel (\autoref{fig:fq_codel}) or fq (\autoref{fig:fq}), Tonopah detects this correctly very often but not in every case. 
...
To evaluate Tonopah's performance under crosstraffic, we use iperf3 with the NewReno congestion control. We start iperf 4\,s before Tonopah so that it has time to saturate the link. 
