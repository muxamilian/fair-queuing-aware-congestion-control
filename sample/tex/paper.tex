...
\begin{figure}[h]
\centering
\subfloat[\footnotesize \textbf{Sending} rate (fair queuing)]{\includegraphics[width=\columnwidth]{{"figures/bw_server_fq"}.pdf}
\label{fig:sender}}\\
\subfloat[\footnotesize \textbf{Receiving} rate (fair queuing)]{\includegraphics[width=\columnwidth]{{"figures/bw_client_fq"}.pdf}
\label{fig:receiver}}
...
\begin{figure}[h]
\centering
\subfloat[\footnotesize \textbf{Sending} rate (no fair queuing)]{\includegraphics[width=\columnwidth]{{"figures/bw_server_pfifo"}.pdf}
\label{fig:sender_pfifo}}\\
\subfloat[\footnotesize \textbf{Receiving} rate (no fair queuing)]{\includegraphics[width=\columnwidth]{{"figures/bw_client_pfifo"}.pdf}
...
When different applications send packets on the internet, one application can send more than the other and thus unfairly take a larger share of bandwidth. 
...
In the following evaluation we always run each network flow for 90\,s. Tonopah measures whether there is fair queuing or not continuously. 
...
However, Tonopah we want to emphasize that Tonopah is not a congestion control algorithm but just a mechanism to measure fair queuing which can be used with any congestion control algorithm. 
...
As crosstraffic we use an iperf with the Newreno congestion control. We start it 4\,s so that it has time to saturate the link. 
